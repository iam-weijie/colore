...
const UserInfo = () => {
 ...
 const fetchDiscoverBoards = async () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetchAPI(`/api/users/getUsers?id=${user!.id}?=maxUsers=10`, {
        method: 'GET'
      })
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      ...
      setForm({ ...form, 
        username: data.username, userLocation: `${data.city}, ${data.state}, ${data.country}` })
      if (
        data.city &&
        data.state &&
        data.country &&
          ...
          if (item.username) {
            ...
            if (response.data[0].user_id != user!.id) {
              ...
              setJoinedCommunities((prev) => [...prev, item.id])
                ...
                setJoinedCommunities((prev) => prev.filter((b) => b != item.id))
              }
            } 
  ...
  const pages = [
    ...
    },
    {
      ...
      caption: "Choose at least one. Find your people! We’ll suggest communities you might like.",
      color: "#CFB1FB",
      ...
      children: (
        <View className="flex-1">
          <FlatList
                        ...
                        ListEmptyComponent={
                          <Text className="text-center text-gray-500">No board to discover</Text>
                        }
