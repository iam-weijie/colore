...
const TabNavigation: React.FC<TabNavigationProps> = ({ name, focused, onPress, notifications }) => {
  return (
    <TouchableOpacity
    ...
    >
      <Text className="text-sm font-[600]" style={{ color: !focused ? "#000000" : "#FBFBFB" }}>
  ...
  const renderFriend = ({ item }: { item: Friendship }) => (
    <View className="flex items-center mb-2 p-4 bg-[#FAFAFA] rounded-[16px]">
        <View className="flex-row justify-between items-center mx-2">
          <TouchableOpacity className="flex-1" activeOpacity={0.6} onPress={() => handleUserProfile(item.friend_id)}>
          ...
          <TouchableOpacity onPress={async () => {
            ...
            try {
                  ...
                  const response: FriendStatusType = await unfriend(user!.id, item.friend_id);
        ...
        {loading ? (
          ...
          <View className="flex-1">
            <View className="flex flex-row justify-between items-center mx-4 mb-4 mt-4">
              ...
              <ScrollView className=" max-h-10" contentOffset={{ x: 10}} horizontal={true} showsHorizontalScrollIndicator={false}>
              <View className="flex flex-row items-center justify-between mx-6">
                <TabNavigation name="Messages" focused={selectedTab === "Messages"} onPress={() => {setSelectedTab("Messages")
setSearchText("")
                }} notifications={toRead.length} />
                <TabNavigation name="Friends" focused={selectedTab === "Friends"} onPress={() => {setSelectedTab("Friends")
setSearchText("")}
            ...
            {selectedTab == "Friends" &&   <FlatList
             ...
             ListHeaderComponent={
              ...
              <TextInput
                ...
                />}
              {selectedTab == "Requests" &&  <View><FlatList
              className="mt-3 mx-4 p-4 bg-[#FAFAFA] rounded-[24px]"
                    data={allFriendRequests?.received}
                    ListHeaderComponent={
                      <View>
                      <Text className="font-JakartaBold text-lg">Requests </Text>
                      <NotificationBubble unread={allFriendRequests?.received ? allFriendRequests.received.length : 0} color="#ffd12b" />
                      </View>
                    }
                    renderItem={renderIncomingRequest}
                    keyExtractor={(item) => item.id.toString()}
                    ListEmptyComponent={
                      <Text className="text-left text-gray-500">No friend requests</Text>
                    }
                  ...
                  />
                    <FlatList
                        ...
                        <NotificationBubble unread={allFriendRequests?.sent ? allFriendRequests.sent.length : 0} color="#ffd12b" />
                        </View>
                      }
