...
const Create = () => {
  ...
  const navigateTo = ({
    ...
    if (
      draftPost.recipient_user_id &&
      params.recipientId &&
      draftPost.recipient_user_id == params.recipientId
    )
      reset = false;
  ...
  const handlePromptSubmit = async () => {
    try {
      const response = await fetchAPI("/api/v2/prompts/getPrompts");
      const uniquePrompts = response.data.filter(
        (value, index, self) =>
          index === self.findIndex((t) => t.cue === value.cue)
      );

      console.log("Fetched prompts:", uniquePrompts);
  ...
  return (
    <View className="flex-1">
      ...
      <EmojiBackground emoji="" color="#93c5fd" />
