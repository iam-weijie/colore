...
const filterPostsByRestrictions = (posts: Post[], board: Board, userId: string): Post[] => {
  return board.restrictions.includes("Everyone")
    ? posts
    : posts.filter((post: Post) => post.recipient_user_id === userId);
};

const filterPersonalPosts = (posts: Post[], isOwnBoard: boolean, viewerId: string): Post[] => {
  return posts.filter((post: Post) => 
    isOwnBoard || post.user_id === viewerId || post.pinned
  );
};

const filterPostsByBoard = (posts: Post[], boardId: number): Post[] => {
  return boardId === -1 ? posts : posts.filter((post: Post) => post.board_id == boardId);
};
...
export const usePersonalPosts = (params: UsePersonalPostsParams) => {
  ...
  const fetchBoardPosts = async (boardId: number, userId: string): Promise<Post[]> => {
    const [boardResponse, postsResponse] = await Promise.all([
      fetchAPI(`/api/boards/getBoardById?id=${boardId}`),
      fetchAPI(`/api/posts/getPostsByBoardId?id=${boardId}`)
    ]);

    return filterPostsByRestrictions(postsResponse.data, boardResponse.data, userId);
  };

  const fetchPersonalPosts = async ( userId: string, viewerId: string): Promise<Post[]> => {
