...
const PersonalBoard: React.FC<PersonalBoardProps> = ({
  ...
  const { boardOnlyPosts, fetchPosts, maxPosts, isLoading, error } =
    usePersonalPosts({
      userId: userId,
      viewerId: user!.id,
      boardId: boardId,
  ...
  const fetchNewPersonalPost = async (excludeIds: number[]) => {
    try {
      ...
      const response = await fetch(
        `/api/v2/posts/getPosts?type=personal&number=1&user_id=${user!.id}&recipient_id=${userId}&exclude_ids=${excludeIdsParam}`
      );
  ...
  return (
    ...
    >
      ...
      <View className="absolute flex-1 inset-0">
        <LinearGradient
