...
const StarringContainer: React.FC<PostContainerProps> = ({
  ...
  const swipeGesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      const dynamicThreshold = threshold + velocityX * 0.3;
      const isLastPost = currentPostIndex === posts.length - 1;
      starScale.value = withTiming(0, { duration: 200 });
      runOnJS(setShowStar)(false);

      if (translateX.value > dynamicThreshold && currentPostIndex > 0) {
        // Swipe right logic remains the same
      } else if (translateX.value < -dynamicThreshold) {
