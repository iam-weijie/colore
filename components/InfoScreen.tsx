...
export const TutorialScreen: React.FC<InfoScreenProps> = ({
  ...
  return (
    <View className={`flex-1 p-2  items-center justify-center mb-6`}>
      {image ? (
        <Animated.View entering={FadeIn.duration(1000)}>
          <Image source={image} className="w-36 h-36 mb-4" resizeMode="contain" />
        </Animated.View>
      ) : (
        <ColoreActivityIndicator
        text='Loading...'
      ...
      {subtitle && (
        <Animated.View entering={FadeInDown.delay(100).duration(800)}>
          <Text className="text-center text-[18px] font-semibold mb-2">{subtitle}</Text>
        </Animated.View>
      )}

      <Animated.View entering={FadeInDown.delay(200).duration(800)}>
        <Text className="text-center text-[16px] text-gray-500 leading-relaxed ">
          {content}
        </Text>
      </Animated.View>
